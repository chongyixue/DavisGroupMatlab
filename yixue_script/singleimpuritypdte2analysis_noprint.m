function [realft_data, imagft_data, symrealft_data, symimagft_data, asymrealft_data, asymimagft_data, test_data, rho] = singleimpuritypdte2analysis(prft_data, qc1, qc2, qr, maskshape,comment)
...
% ymin = min(0,min(arp2)-0.1*abs(min(arp2)));
% ymax = max(max(arp2)+0.1*abs(max(arp2)),0);
% figure, plot(pe, arp2,'-o','color','b', 'LineWidth', 2, 'MarkerSize', 10);
% hold on;
...
% box on
% autosave
% savepath = 'C:\Users\Rahul\Documents\MATLAB';
...
% else
% 
%    figureset_img_plot(asymrealmap(:,:,zlayer-6))
