...
bigdataholder = bigdata;

clear list;
for p=1:10
    for m = 1:energypoints
        for n = 1:ndata
            list = bigdataholder(n).rho(p).arp1(m);
            antireal(p,m,n)=list;
            list = bigdataholder(n).rho(p).aip1(m);
            antiimag(p,m,n)=list;
            list = bigdataholder(n).rho(p).srp1(m);
            symreal(p,m,n)=list;
            list = bigdataholder(n).rho(p).sip1(m);
            symimag(p,m,n)=list;
        end
    end
end
...
datamatrix.antireal(1:rows,1:10,1:10)=0;%index 3 is number of points
datamatrix.antiimag(1:rows,1:10,1:10)=0;%index 3 is number of points
datamatrix.symreal(1:rows,1:10,1:10)=0;%index 3 is number of points
datamatrix.symimag(1:rows,1:10,1:10)=0;%index 3 is number of points
...
for p = 10:10
    ...
    savepath = 'C:\Users\chong\Documents\MATLAB\STMdata\PdTe2\HAEM\';
    comment = strcat({'combinedHAEMpoint'},{num2str(p)},'antireal');
    slstr = strcat(savepath,comment,'.','png');
    set(gcf, 'PaperPositionMode', 'auto');
    saveas(gcf, slstr{1}, 'png');
    ...
    savepath = 'C:\Users\chong\Documents\MATLAB\STMdata\PdTe2\HAEM\';
    comment = strcat({'combinedHAEMpoint'},{num2str(p)},'antiimag');
    slstr = strcat(savepath,comment,'.','png');
    set(gcf, 'PaperPositionMode', 'auto');
    saveas(gcf, slstr{1}, 'png');
    ...
    savepath = 'C:\Users\chong\Documents\MATLAB\STMdata\PdTe2\HAEM\';
    comment = strcat({'combinedHAEMpoint'},{num2str(p)},'symreal');
    slstr = strcat(savepath,comment,'.','png');
    set(gcf, 'PaperPositionMode', 'auto');
    saveas(gcf, slstr{1}, 'png');
    ...
    savepath = 'C:\Users\chong\Documents\MATLAB\STMdata\PdTe2\HAEM\';
    comment = strcat({'combinedHAEMpoint'},{num2str(p)},'symimag');
    slstr = strcat(savepath,comment,'.','png');
    set(gcf, 'PaperPositionMode', 'auto');
    saveas(gcf, slstr{1}, 'png');
