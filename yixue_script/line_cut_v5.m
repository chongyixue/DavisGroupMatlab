...
% 170929 YXC Added pixel as distance unit

function l_cut = line_cut_v5(data,pos1,pos2,avg_px)
%avg_px = 2;
lcut = [];
if isstruct(data)
    img = data.map;
...
if isstruct(data)
    x = data.r;
    if strcmp(data.coord_type, 'r') == 1
        if dx == 0
            r = x(1 : step_n);
        elseif dy == 0
            r = x(1 : step_n);
        else
            x1 = sqrt( ( x(pos1(1))- x(pos2(1)) )^2 + ( x(pos1(2))- x(pos2(2)) )^2);
