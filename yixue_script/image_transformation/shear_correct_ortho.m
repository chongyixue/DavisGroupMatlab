...
function [newmap,q0,p,theta,alpha] = shear_correct_ortho(map,Bragg1,Bragg2,varargin)
...
%% functions
    ...
    function F2 = root4d(x,Bgs)
        ...
        q2x = q0*cos(alpha+pi/2)+p*q0*sin(theta)*(cos(theta-alpha-pi/2))-Bgs(2,1);
        q2y = q0*sin(alpha+pi/2)-p*q0*cos(theta)*(cos(theta-alpha-pi/2))-Bgs(2,2);
