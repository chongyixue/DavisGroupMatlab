...
for iH = 1:nhor
    for iV = 1:nvert
        ...
        % locrhos loccdws for sum of loc in for all 3 directions
...
%             setlim = 1; 
        ...
        elseif strcmp(plotguide(1,1:3),'non')
            ...
            printf(strcat('"',plotguide(1,:),'" is not recognized as one of the available plots'));
            datatype = 404;
            data = [];
        end
        ...
        elseif datatype == 2 % blackwhite inphase check
            ...
            %             'Units','pixels',...
            %             'Position',[left bottom siz siz],...
            %             'String',str,'BackgroundColor',[0.8 0.8 0.8],...
            %             'FontSize',14);
...
%                 size(data,1)
        ...
        elseif datatype == 4 % histogram type
            data = reshape(data,1,[]);
...
%     figure,
%     scatter1 = scatter(chargepoints(i,:),rhopoints(i,:),'r.');
%     scatter1.MarkerFaceAlpha = .2;
%     scatter1.MarkerEdgeAlpha = .2;
%     scatter1.SizeData = 1;
%
%     title(strcat('Phase scatter plot Bragg ',num2str(i),' rsig=',num2str(sig)));
%     xlabel("charge phase")
%     ylabel("pair phase")
