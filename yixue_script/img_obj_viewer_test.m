...
% addition 2019-7-8 for spectrum viewer axes to be fixed scale
maxx = max(max(max(in_data.map)));
minn = min(min(min(in_data.map)));
...
coord_tb = uicontrol('Style','text',...                
   ...
   % initialize variables in case we need to use for deeper calls 
   inner_enlist = 0;
   inner_f = 0;
   fh_temp = 0;
...
f0b = uimenu(f0,'Label','Save Layer to Workspace','Callback',@save_lyr_Callback);
    ...
    uimenu(f0,'Label','Export Movie Express','Callback',@export_movie_express_Callback);
...
disp = uimenu('Label','Display');
    ...
    uimenu(disp,'Label','Show normalized FFT avg spec','Callback',@avg_spect_FFT_Callback);  
process = uimenu('Label','Process');
    ...
    crop_menu = uimenu(process,'Label','Crop');
        ...
        uimenu(crop_menu,'Label','Crop by choosing Center', 'Callback',@crop_center_Callback)
    ...
    linecut_menu = uimenu(process,'Label','Line Cut');
        uimenu(linecut_menu,'Label','Arbitrary Cut','Callback',@line_cut_Callback);
        radialcut = uimenu(linecut_menu,'Label','Radial Cut');
            uimenu(radialcut,'Label','All','Callback',@radial_cut_Callback);
            uimenu(radialcut,'Label','Single','Callback',@radial_cut_single_Callback);
        symmetric_menu = uimenu(linecut_menu,'Label','Symmetric Cut');
            uimenu(symmetric_menu,'Label','All','Callback',@line_cut_symmetric_Callback);
            uimenu(symmetric_menu,'Label','Single layer','Callback',@line_cut_single_layer_Callback);
    ...
    img_manip_menu = uimenu(process,'Label','Image Manipulation');
        ...
        uimenu(img_manip_menu,'Label','Six-fold-Symm','Callback',@sixfold_Callback);
...
analysis = uimenu('Label','Analysis');
    ...
    maskFTpeak = uimenu(analysis,'Label','Mask FT peaks');
        uimenu(maskFTpeak,'Label','Draw','Callback',@maskFTpeak_Callback);
        uimenu(maskFTpeak,'Label','Coordinates','Callback',@maskFTpeak_coordinate_Callback);
    inversemaskFTpeak = uimenu(analysis,'Label','Inverse Maks FT peaks');
        uimenu(inversemaskFTpeak,'Label','Draw','Callback',@inversemaskFTpeak_Callback);
        uimenu(inversemaskFTpeak,'Label','Coordinates','Callback',@inversemaskFTpeak_coordinate_Callback);
    invertPeak = uimenu(analysis,'Label','IFT of q-peaks');
        uimenu(invertPeak,'Label','2 peaks','Callback',@Peak2_Callback);
        uimenu(invertPeak,'Label','4 peaks','Callback',@Peak4_Callback);
        uimenu(invertPeak,'Label','6 peaks','Callback',@Peak6_Callback);
     uimenu(analysis,'Label','Spline gapmap','Callback',@splinefit_Callback);
...
%         end
    ...
    function avg_spect_FFT_Callback(hObject,eventdata)
        data = guidata(f);
        x = data.e*1000;
        y = data.ave;
        ...
        layers = nz;
        dq = abs(pi/(Dx));
        q =((1:(round((layers+1)/2)))-1)'*dq;
        spec = y;
        m = mean(spec);
        spec = spec - m;
        fo = abs(fft(spec));
        fo = fo/max(fo);
        subfo = fo(1:(layers+1)/2);
        figure,
        plot(q,subfo)
        title('Single sided FFT of average spectrum')
        xlabel('mV$^{-1}$','Interpreter','latex')
    end
    ...
    function export_movie_Callback(hObject,eventdata)
        ...
        %         filename = 'testtest20180923';
        %         exp_movie_new(data.map,data.e,data.r,data.r,export_path,filename,c_map,color_lim,6,100);
    end
%added 2018-9-23 express saving without asking more details
    function export_movie_express_Callback(hObject,eventdata)
        data = guidata(f);
        color_lim = get(f,'UserData');
        %color_lim = color_lim(layer,:);
        c_map =  get(f,'Colormap');
        %exp_movie_dialogue(data.map,data.e,data.r,data.r,c_map,color_lim);
        export_path = 'C:\Users\chong\Documents\MATLAB\STMdata\MoTeSe\';
        filename = ['auto','data.name','data.var'];
        exp_movie_new(data.map,data.e,data.r,data.r,export_path,filename,c_map,color_lim,6,100);
    end
    ...
    function crop_center_Callback(hOjbect,eventdata)
        data = guidata(f);
        crop_center_dialogue(data);
    end
...
%         line_cut_v4(data);
    ...
    function line_cut_symmetric_Callback(hObject,eventdata)
        data = guidata(f);
        line_cut_core_dialogue(data);
    end
    function radial_cut_Callback(hObject,eventdata)
        data = guidata(f);
        radial_dialogue(data);
    end
    function radial_cut_single_Callback(hObject,eventdata)
        data = guidata(f);
        layer=get(energy_list,'Value'); 
        radial_dialogue_single(data,layer);
    end
    function line_cut_single_layer_Callback(hObject,eventdata)
        data = guidata(f);
        layer=get(energy_list,'Value'); 
        line_cut_core_single_layer_dialogue(data,layer);
    end
    ...
    function sixfold_Callback(hObject,eventdata)
        data = guidata(f);
        sixfold_dialogue2(data);
    end
    ...
    % 2019-9-21
    function maskFTpeak_Callback(hObject,eventdata)
       data = guidata(f);
    ...
    function inversemaskFTpeak_Callback(hObject,eventdata)
        data = guidata(f);
    ...
    function maskFTpeak_coordinate_Callback(hObject,eventdata)
        data = guidata(f);
        
       %first get amplitude FT displayed
       map = data;    
       img_obj_viewer_test(remove_peak_coord(data))
       
    end
    function inversemaskFTpeak_coordinate_Callback(hObject,eventdata)
        data = guidata(f);
    ...
    function Peak2_Callback(hObject,eventdata)
       data = guidata(f);
       Peak_invert_dialogue(data,2);
    end
    function Peak4_Callback(hObject,eventdata)
       data = guidata(f);
       Peak_invert_dialogue(data,4);
    end
    function Peak6_Callback(hObject,eventdata)
       data = guidata(f);
       Peak_invert_dialogue(data,6);
    end
    function splinefit_Callback(hObject,eventdata)
        data = guidata(f);
        splinefit_gapmap_dialogue(data);
    end
...
subback0map = polyn_subtract(map,0,3); %something on varargin(3):no-popup
close(fh_temp)
%keep complex FT for calculation
% FTcomplex = fourier_transform2d(subback0map,'none','complex','ft');
FTcomplex = fourier_transform2d(map,'none','complex','ft');
newdata = maskFTpeak_dialogue(FTcomplex,layer,color_lim,c_map);
...
% FTcomplex = fourier_transform2d(subback0map,'none','complex','ft');
FTcomplex = fourier_transform2d(map,'none','complex','ft');
newdata = inversemaskFTpeak_dialogue(FTcomplex,layer,color_lim,c_map);
img_obj_viewer_test(newdata);
