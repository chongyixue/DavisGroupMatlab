...
function newmap2 = scale_to_match(map1,pa1,pa2,map2,pb1,pb2)
    ...
    fun = @(x) simfour(pa1,pa2,pb1,pb2,x);
    x0 = [1,1,0,0];
    x = fsolve(fun,x0);
...
%     M
    tform = affine2d(M);
    centerOutput = affineOutputView(size(map1.map),tform,'BoundsStyle','CenterOutput');
    newmap2 = map2;
    newmap2.r = map1.r;
    ...
    function F = simsix(pa1,pa2,pa3,pb1,pb2,pb3,x)
        ...
        e = x(5);
        f = x(6);
        F(1) = a*pb1(1)+b*pb1(2)+e-pa1(1);
        F(2) = a*pb2(1)+b*pb2(2)+e-pa2(1);
        F(3) = a*pb3(1)+b*pb3(2)+e-pa3(1);
        F(4) = c*pb1(1)+d*pb1(2)+f-pa1(2);
        F(5) = c*pb2(1)+d*pb2(2)+f-pa2(2);
        F(6) = c*pb3(1)+d*pb3(2)+f-pa3(2);        
    end
