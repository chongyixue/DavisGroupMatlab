...
% for (x,y) = (1,1):
peakmV_array = [0,0,102,145,185,202,220,234,0,0,0,0];
e_array = [-1,0,1,2,3]/1000;
background = 2;

ll = fitted_to_viewer(map,llmatrix,background,e_array,'90228a00_llmap_splineminima',peakmV_array);
ll_0 = fitted_to_viewer(map,llmatrix,1,e_array,'90228a00_llmap_no_background',peakmV_array);
ll_1 = fitted_to_viewer(map,llmatrix,2,e_array,'90228a00_llmap_3pointspline',peakmV_array);
...
llmap_combine_filtered = llmap_combine;
llmap_combine_filtered.map = replace_with_neighbors(llmap_combine_filtered.map,1);
% for lay = 1:nz
%     llmap_combine_filtered.map(:,:,lay) = removepeaks(llmap_combine.map(:,:,lay),10);
% end
llmap_combine_filtered.name = 'filtered';
img_obj_viewer_test(llmap_combine_filtered);
