...
% for (x,y) = (1,1):
peakmV_array = [0,0,0,100,137,177,197,0,0];
e_array = [-1,0,1,2]/1000;
background = 2;

map = invert_map(obj_90311A00_G);
ll = fitted_to_viewer(map,llmatrix,background,e_array,'90311a00_llmap_splineminima',peakmV_array);
ll_0 = fitted_to_viewer(map,llmatrix,0,e_array,'90311a00_llmap_no_background',peakmV_array);
ll_1 = fitted_to_viewer(map,llmatrix,1,e_array,'90311a00_llmap_3pointspline',peakmV_array);
...
llmap_combine.name = '90311a00_llmap_combined';
...
llmap_combine_filtered_90311 = llmap_combine;
llmap_combine_filtered_90311.map = replace_with_neighbors(llmap_combine_filtered_90311.map,1);
% for lay = 1:nz
%     llmap_combine_filtered.map(:,:,lay) = removepeaks(llmap_combine.map(:,:,lay),10);
% end
llmap_combine_filtered_90311.name = 'filtered_90311';
img_obj_viewer_test(llmap_combine_filtered_90311);

filtered_peakarray = peakmV_array;
range = [10,10,10,10];
llmap_combine_filtered_90311 = filter_misfits(llmap_combine_filtered_90311,peakmV_array,range);
img_obj_viewer_test(llmap_combine_filtered_90311);
