...
ll = fitted_to_viewer(map,llmatrix,background,e_array,'90411a00_llmap_splineminima',peakmV_array);
ll_0 = fitted_to_viewer(map,llmatrix,0,e_array,'90411a00_llmap_no_background',peakmV_array);
ll_1 = fitted_to_viewer(map,llmatrix,1,e_array,'90411a00_llmap_3pointspline',peakmV_array);

% img_obj_viewer_test(ll);
% img_obj_viewer_test(ll_1);
% img_obj_viewer_test(ll_0);
[~,~,nz] = size(ll.map);
llmap_combine_90411 = ll;
for lay = 1:nz
    measure = ll.map(:,:,lay) == 0;
    llmap_combine_90411.map(:,:,lay) = (1-measure).*ll.map(:,:,lay) + ll_1.map(:,:,lay).*measure;
    measure = llmap_combine_90411.map(:,:,lay) == 0;
    llmap_combine_90411.map(:,:,lay) = (1-measure).*llmap_combine_90411.map(:,:,lay) + ll_0.map(:,:,lay).*measure;
end
llmap_combine_90411.name = '90411a00_llmap_combined';

img_obj_viewer_test(llmap_combine_90411);





llmap_combine_filtered_90411 = llmap_combine_90411;
llmap_combine_filtered_90411.map = replace_with_neighbors(llmap_combine_filtered_90411.map,1);
% for lay = 1:nz
%     llmap_combine_filtered.map(:,:,lay) = removepeaks(llmap_combine.map(:,:,lay),10);
% end
llmap_combine_filtered_90411.name = 'filtered_90411';
llmap_combine_filtered_90411.method = 5;

filtered_peakarray = peakmV_array;
range = 5;
llmap_combine_filtered_90411 = filter_misfits(llmap_combine_filtered_90411,peakmV_array,range);
img_obj_viewer_test(llmap_combine_filtered_90411);
