...
%2017/10/23 Yi Xue Chong, Rahul Sharma
...
function [newcx,newcy,imagmat] = imagHAEM_vary_plot_topo(topo,cx,cy,pix_range,sstep)
...
for i = -max_steps:max_steps
    for j = -max_steps:max_steps
        imagmat(j+max_steps+1,i+max_steps+1)=abs(imagpartHAEMtopo(cx+i*sstep,cy+j*sstep,topo));
    end
end
