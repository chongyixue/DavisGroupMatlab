...
for i=1:length(OneTx)
    if OneTx(i) <=185 || OneTx(i) > 370 || OneTy(i) <=0 || OneTy(i) >185
    else
        OneTxx(t)=OneTx(i);
        OneTyy(t)=OneTy(i);
        t=t+1;
...
for i=1:length(ThreeTx)
    if ThreeTx(i) <=185 || ThreeTx(i) > 370 || ThreeTy(i) <=0 || ThreeTy(i) >185
    else
...
for i=1:length(FourTx)
    if FourTx(i) <=185 || FourTx(i) > 370 || FourTy(i) <=0 || FourTy(i) >185
    else
        FourTxx(t)=FourTx(i);
        FourTyy(t)=FourTy(i);
        t=t+1;
...
for i=1:length(SixTx)
    if SixTx(i) <=185 || SixTx(i) > 370 || SixTy(i) <=0 || SixTy(i) >185
    else
        SixTxx(t)=SixTx(i);
        SixTyy(t)=SixTy(i);
        t=t+1;
...
for i=1:length(EightTx)
    if EightTx(i) <=185 || EightTx(i) > 370 || EightTy(i) <=0 || EightTy(i) >185
    else
...
% Topon=Topo(1:185,186:370);
% Toponm=Topon;
% Toponmm=Topon;

% for z=1:100
    for i =1:length(AllTxx)
        x0 = [1; AllTxx(i)-185; 3; AllTyy(i); 3; 0; 0];
