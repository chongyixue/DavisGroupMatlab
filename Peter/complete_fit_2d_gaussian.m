function [x,resnorm,residual]=complete_fit_2d_gaussian(data, varargin)
...
x0 = [1; comx; 1; comy; 1; 1; mean(mean(zdata))];
...
% lb=[min(min(zdata)); 1; 0; 1; 0; -inf; -pi/2]; 
lb=[-max(max(zdata)); 1; 0; 1; 0; -inf; -pi/2]; 
ub=[max(max(zdata)); max(size(data(:,:,1),1)); max(size(data(:,:,1),1));...
    max(size(data(:,:,1),2)); max(size(data(:,:,1),2)); max(max(zdata)); pi/2]; 
% lb=[-inf; 1; 0; 1; 0; -inf; -pi/2]; 
% ub=[inf; max(size(data(:,:,1),1)); max(size(data(:,:,1),1));...
%     max(size(data(:,:,1),2)); max(size(data(:,:,1),2)); inf; pi/2];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[x,resnorm,residual]=lsqcurvefit(@twodgauss,x0,xdata,zdata,lb,ub,options);
