...
exp_lrntz = 'a*exp(-b*x) + c*(f/2)^2/((x-d)^2+(f/2)^2)+g*(k/2)^2/((x-h)^2+(k/2)^2)+l';
...
%     lorent(i)=gg(1)*exp(-gg(2)*x(i))+gg(9);
%     lorent2(i)=peak1(i)+peak2(i)+lorent(i);
%     allfct(i)=gg(1)*exp(-gg(2)*x(i)) + gg(3)*(gg(5)/2)^2/((x(i)-gg(4))^2+(gg(5)/2)^2)+g*(k/2)^2/((x-h)^2+(k/2)^2)+l;
%     peak2(i)=gg(6)/((x(i)-gg(7))^2+gg(8)^2);
end
y_new = feval(p,x);
figure, plot(x,y,'.k',x,y_new,'-r',x,peak1,'-b',x,peak2,'-r','linewidth',2);
