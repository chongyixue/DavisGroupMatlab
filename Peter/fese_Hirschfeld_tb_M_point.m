function [eigeps2D, orbcha] = fese_Hirschfeld_tb_M_point(pixelnum, kz)
...
kx = linspace(-0.1*pi, 0.1*pi, n+1);
ky = linspace(pi-0.1*pi, pi+0.1*pi, n+1);
...
for k=1:n+1
    for l=1:n+1
        ...
        H1 = Hirschfeld_tb_hamiltonian_M_point(px, py, p1, p2, kz);
        H(:,:,m) = H1(:,:,1);
