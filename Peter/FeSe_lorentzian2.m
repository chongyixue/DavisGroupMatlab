function [y_new, p,gof, ci]=FeSe_lorentzian2(y,range,guess,low,upp)
% range is [left right]
...
% exp_lrntz = 'a+ b*x + c*(x-d)^2 + e*(f/2)^2/((x-g)^2+(f/2)^2)+h*(j/2)^2/((x-k)^2+(j/2)^2)';
...
y_new = feval(p,x);
figure, plot(x,y,'.k',x,y_new,'-r',x,peak1,'-b', x,peak2,'-g','linewidth',2);
legend('data','fit','peak1', 'peak2');
% pause(0.5);
end