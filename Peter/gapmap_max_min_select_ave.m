function sigmax = gapmap_max_min_select_ave(spec,ev,specd,evd,maxi,mini,maxid,minid)
...
%     line([dum8 dum8],[-1 1],'LineWidth',2,'Color','r');
    ...
    %% Determine if at least three of the four criteria are above threshold,
...
%         [minbet,minbetind]=min(spec(eoam:maxi(i)));
%         if abs(minbet - min( [mispvl(tc), mispvh(tc)]) ) > 0.1
%             mon(tc) = 0;
%         end
%     end
