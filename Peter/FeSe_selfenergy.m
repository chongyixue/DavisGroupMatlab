function [y_new, p,gof, ci]=FeSe_selfenergy(y,range,guess,low,upp)
% range is [left right]
...
exp_lrntz = 'a + b*(x-c) + real( h / sqrt(2*k + 1i*2*l - x) )';

% exp_lrntz = 'real( a / sqrt(2*b + 1i*2*c - x) ) + d + e*x + f * exp(-g*(x-h))';
