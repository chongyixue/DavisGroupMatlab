function [Vseq,Dseq,Vcell, Dcell] = eigenshuffle2D(Asequence, nx)
...
%% changed by me 2/23/2016 tryin to accomodate 2D eigenvalue problem
...
%%%%%
  ...
  % Is there a best permutation?
  reorder = munkres(dist);
  reorder1 = munkres(dist1);
  reorder2 = munkres(dist2);
  
  Vseq(:,:,2) = Vseq(:,reorder,2);
  Dseq(:,2) = Dseq(reorder,2);
  
  % also ensure the signs of each eigenvector pair
  % were consistent if possible
  S = squeeze(real(sum(Vseq(:,:,1).*Vseq(:,:,2),1))) < 0;
  Vseq(:,S,2) = -Vseq(:,S,2);
...
%%%%%%%
  ...
  Vseq(:,:,nx+1) = Vseq(:,reorder,nx+1);
  Dseq(:,nx+1) = Dseq(reorder,nx+1);
  
  % also ensure the signs of each eigenvector pair
  % were consistent if possible
  S = squeeze(real(sum(Vseq(:,:,2).*Vseq(:,:,nx+1),1))) < 0;
  Vseq(:,S,nx+1) = -Vseq(:,S,nx+1);
...
for i = 3:nx
      ...
      Vseq(:,:,v2c) = Vseq(:,reorder,v2c);
      Dseq(:,v2c) = Dseq(reorder,v2c);

      % also ensure the signs of each eigenvector pair
      % were consistent if possible
      S = squeeze(real(sum(Vseq(:,:,v1c).*Vseq(:,:,v2c),1))) < 0;
      Vseq(:,S,v2c) = -Vseq(:,S,v2c);
...
for i = 2:nx-1
    ...
    V1 = Vseq(:,:,v1c);
      ...
      Vcell{i+1,1} = Vseq(:,:,v2c);
      Dcell{i+1,1} = Dseq(:,v2c);
end
%%%%%%%%%     
for i = 2:nx
    ...
    adum2 = ( sum(dum3, 2)) /d3;
    [DX, DY] = ndgrid(dum2, adum2);
    [B, I] = min( abs (DX - DY),[],2 );
    
    reorder = munkres(abs (DX - DY));
