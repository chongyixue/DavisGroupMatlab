function hh = verrorbar(x, y, l, u, symbol)
%VERRORBAR Vertical Error bar plot.
% 06/19/2016 - copied by Peter and just changed for purpose of plotting
% vertical error bars the same way as in herrorbar
%   VERRORBAR(X,Y,L,R) plots the graph of vector X vs. vector Y with
...
%% changed by Peter 06/19/2016
tee = abs(ytop-ybot)/10; % make tee .02 x-distance for error bars
tee = 0.005;
xl = x - tee;
xr = x + tee;
...
% xr = x + u;
% %% changed by Peter 06/19/2016
% tee = abs(xr-xl)/100; % make tee .02 x-distance for error bars
% 
% ytop = y + tee;
% ybot = y - tee;
% n = size(y,2);
