...
for i=1:length(OneTx)
    if OneTx(i) <=200 || OneTx(i) > 512 || OneTy(i) <=200 || OneTy(i) >512
    else
        OneTxx(t)=OneTx(i);
        OneTyy(t)=OneTy(i);
        t=t+1;
...
for i=1:length(ThreeTx)
    if ThreeTx(i) <=200 || ThreeTx(i) > 512 || ThreeTy(i) <=200 || ThreeTy(i) >512
    else
...
for i=1:length(FourTx)
    if FourTx(i) <=200 || FourTx(i) > 512 || FourTy(i) <=200 || FourTy(i) >512
    else
        FourTxx(t)=FourTx(i);
        FourTyy(t)=FourTy(i);
        t=t+1;
...
for i=1:length(SixTx)
    if SixTx(i) <=200 || SixTx(i) > 512 || SixTy(i) <=200 || SixTy(i) >512
    else
        SixTxx(t)=SixTx(i);
        SixTyy(t)=SixTy(i);
        t=t+1;
...
for i=1:length(EightTx)
    if EightTx(i) <=200 || EightTx(i) > 512 || EightTy(i) <=200 || EightTy(i) >512
    else
...
topo=Topo(201:512,201:512);
Topon=boxestoones(topo);

[xcor,ycor,xcor2,ycor2,Cocounter,schab]=boxesNAatoms(topo,Topon);
Topon=schab;
...
for z=1:100
    for i =1:length(AllTxx)
        x0 = [1; AllTxx(i)-200; 3; AllTyy(i)-200; 3; 0; 0];
...
%                 x = nx/2+round(R(j)*cos(theta(k)));
%                 y = ny/2-round(R(j)*sin(theta(k)));
