


eg_peter = linspace(-2.1,-0.8,14);
xg_peter = [0.122,0.10948,0.10304,0.10304,0.10304,0.0966,0.10304,0.0966,0.09016,0.08372,0.07728,0.07084,0.0644,0.0644];
yg_peter = abs([-0.077,-0.10304,-0.10948,-0.11592,-0.12236,-0.1288,-0.14168,-0.15456,-0.15456,-0.161,-0.161,-0.16744,-0.16744,-0.17388]);

xg1 = xg_peter;
yg1 = yg_peter;

% em = linspace(-0.9,-0.50,5);
% 
% 
% xm = [-0.262,-0.292,-0.314,-0.337,-0.359];
% ym = [-0.082,-0.082,-0.075,-0.067,-0.060];
% 
% xm_peter = [0.26964,0.28462,0.31458,0.33705,0.34454];
% ym_peter = [-0.08239,-0.0749,-0.06741,-0.05992,-0.05243];
% 
% xm_peter2 = 0.23219;
% ym_peter2 = -0.08239;
% 
% xm_peter3 = [0.10989,0.15984,0.18981,0.22977,0.24975,0.25974,0.28971,0.28971,0.31968,0.34968,0.36963,0.36963];
% ym_peter3 = [-0.05994,-0.05994,-0.06993,-0.06993,-0.06993,-0.06993,-0.05994,-0.05994,-0.06993,-0.07992,-0.04995,0];
% 
% %xm_peter3 = [0.22977,0.24975,0.25974,0.28971,0.28971,0.31968,0.33966,0.35964,0.36963];
% %ym_peter3 = [-0.06993,-0.06993,-0.06993,-0.05994,-0.05994,-0.06993,-0.07992,-0.04995,0];
% 
% % xm_peter4 = [0.35964, 0.36963];
% % ym_peter4 = [-0.04995, 0];
% 
% xm_peter4 = [0.36963];
% ym_peter4 = [0];

% xm_error = [0.02247,0.02247,0.02247,0.02247,0.02247];
% 
% xm_error2 = 0.02247;
% 
% xm_error4 = [0.02997];
% 
% % xm_error4 = [0.02997, 0.02997];

figure(1),


p = 3;


line([-0.025,0.175],[0,0],'Color','k','LineStyle','--','LineWidth',2)
line([0,0],[-0.025,0.225],'Color','k','LineStyle','--','LineWidth',2)


%%
xg_error = [0.02576,0.01932,0.02576,0.02576,0.02576,0.02576,0.01932,0.02576,0.02576,0.01932,0.01932,0.01932,0.02576,0.02576];

%%
crad = 0.005;
rectangle('Position',[xg1(2)-crad/2,yg1(2)-crad/2,crad,crad],'Curvature',[1,1],...
          'FaceColor','b','LineStyle','none')
      
% plot(xg1(1), yg1(1),'ok', 'MarkerSize', p,'MarkerFaceColor','k');      
hold on

verrorbar(xg1(2), yg1(2), xg_error(2),'k');
herrorbar(xg1(2), yg1(2), xg_error(2),'k');

eg_peter = linspace(-2.0,-0.8,13);

for i=2 : length(xg1)-1
    if abs(eg_peter(i)) == 1.5
        rectangle('Position',[xg1(i)-crad/2,yg1(i)-crad/2,crad,crad],'Curvature',[1,1],...
              'FaceColor','r','LineStyle','none')
    %     plot(xg1(i), yg1(i),'ok', 'MarkerSize', p,'MarkerFaceColor','k');    
        verrorbar(xg1(i), yg1(i),  xg_error(i),'k');
        herrorbar(xg1(i), yg1(i),  xg_error(i),'k');  
    end
    if abs(eg_peter(i-1)) == 1.0
        rectangle('Position',[xg1(i)-crad/2,yg1(i)-crad/2,crad,crad],'Curvature',[1,1],...
              'FaceColor','g','LineStyle','none')
    %     plot(xg1(i), yg1(i),'ok', 'MarkerSize', p,'MarkerFaceColor','k');    
        verrorbar(xg1(i), yg1(i),  xg_error(i),'k');
        herrorbar(xg1(i), yg1(i),  xg_error(i),'k');  
    end
    if abs(eg_peter(i-1)) == 1.0
        rectangle('Position',[xg1(i)-crad/2,yg1(i)-crad/2,crad,crad],'Curvature',[1,1],...
              'FaceColor','m','LineStyle','none')
    %     plot(xg1(i), yg1(i),'ok', 'MarkerSize', p,'MarkerFaceColor','k');    
        verrorbar(xg1(i), yg1(i),  xg_error(i),'k');
        herrorbar(xg1(i), yg1(i),  xg_error(i),'k');  
    end
end


for i=1 : 11
    dummy = q1_lorpar(i,1:4);
    if dummy(4) == 2.0
        rectangle('Position',[-crad/2,dummy(3)-crad/2,crad,crad],'Curvature',[1,1],...
              'FaceColor','b','LineStyle','none')
        verrorbar(0, dummy(3),  dummy(2)/2,'k'); 
    end
    if dummy(4) == 1.5
        rectangle('Position',[-crad/2,dummy(3)-crad/2,crad,crad],'Curvature',[1,1],...
              'FaceColor','r','LineStyle','none')
        verrorbar(0, dummy(3),  dummy(2)/2,'k'); 
    end
    if dummy(4) == 1.0
        rectangle('Position',[-crad/2,dummy(3)-crad/2,crad,crad],'Curvature',[1,1],...
              'FaceColor','g','LineStyle','none')
        verrorbar(0, dummy(3),  dummy(2)/2,'k'); 
    end
    if dummy(4) == 1.0
        rectangle('Position',[-crad/2,dummy(3)-crad/2,crad,crad],'Curvature',[1,1],...
              'FaceColor','m','LineStyle','none')
        verrorbar(0, dummy(3),  dummy(2)/2,'k'); 
    end
end    
    
for i=1 : 15
    dummy = q3_lorpar(i,1:4);
     if dummy(4) == 2.4
        rectangle('Position',[dummy(3)-crad/2,-crad/2,crad,crad],'Curvature',[1,1],...
              'FaceColor','k','LineStyle','none')
        herrorbar(dummy(3), 0,  dummy(2)/2,'k');   
     end
    
    if dummy(4) == 2.0
        rectangle('Position',[dummy(3)-crad/2,-crad/2,crad,crad],'Curvature',[1,1],...
              'FaceColor','b','LineStyle','none')
        herrorbar(dummy(3), 0,  dummy(2)/2,'k');   
    end
     if dummy(4) == 1.5
        rectangle('Position',[dummy(3)-crad/2,-crad/2,crad,crad],'Curvature',[1,1],...
              'FaceColor','r','LineStyle','none')
        herrorbar(dummy(3), 0,  dummy(2)/2,'k');   
     end
     if dummy(4) == 1.0
        rectangle('Position',[dummy(3)-crad/2,-crad/2,crad,crad],'Curvature',[1,1],...
              'FaceColor','g','LineStyle','none')
        herrorbar(dummy(3), 0,  dummy(2)/2,'k');   
     end
     if dummy(4) == 1.0
        rectangle('Position',[dummy(3)-crad/2,-crad/2,crad,crad],'Curvature',[1,1],...
              'FaceColor','m','LineStyle','none')
        herrorbar(dummy(3), 0,  dummy(2)/2,'k');   
     end
    
end  

hold off


%hold off
axis equal
axis([-0.025 0.175 -0.025 0.225]);
%axis(2*[-0.25 0.25 -0.25 0.25]);
%axis(2*[-0.25 0 -0.25 0.25]);
ax2 = gca;
ax2.FontSize = 20;
ax2.TickLength = [0.02 0.02];
ax2.LineWidth = 2;
ax2.XTick = linspace(-0.025,0.175,5);
ax2.YTick = linspace(-0.025,0.225,5);
%axis square
%axis off
%ax2.XTick = linspace(0,90,7);
%xlabel('q_x (A^{-1})','FontSize',20);
%ylabel('q_y (A^{-1})','FontSize',20);

figsiz=[25 25];
fig_siz(figsiz);
%pbaspect([1.25 1 1])

figw
box on
