...
for i=1:length(OneTx)
    if OneTx(i) <=0 || OneTx(i) > 185 || OneTy(i) <=0 || OneTy(i) >185
    else
        OneTxx(t)=OneTx(i);
        OneTyy(t)=OneTy(i);
        t=t+1;
...
for i=1:length(ThreeTx)
    if ThreeTx(i) <=0 || ThreeTx(i) > 185 || ThreeTy(i) <=0 || ThreeTy(i) >185
    else
...
for i=1:length(FourTx)
    if FourTx(i) <=0 || FourTx(i) > 185 || FourTy(i) <=0 || FourTy(i) >185
    else
        FourTxx(t)=FourTx(i);
        FourTyy(t)=FourTy(i);
        t=t+1;
...
for i=1:length(SixTx)
    if SixTx(i) <=0 || SixTx(i) > 185 || SixTy(i) <=0 || SixTy(i) >185
    else
        SixTxx(t)=SixTx(i);
        SixTyy(t)=SixTy(i);
        t=t+1;
...
for i=1:length(EightTx)
    if EightTx(i) <=0 || EightTx(i) > 185 || EightTy(i) <=0 || EightTy(i) >185
    else
