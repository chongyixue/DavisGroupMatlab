...
for i=1:length(OneTx)
    if OneTx(i) <=0 || OneTx(i) > 312 || OneTy(i) <=200 || OneTy(i) >512
    else
        OneTxx(t)=OneTx(i);
        OneTyy(t)=OneTy(i);
        t=t+1;
...
for i=1:length(ThreeTx)
    if ThreeTx(i) <=0 || ThreeTx(i) > 312 || ThreeTy(i) <=200 || ThreeTy(i) >512
    else
...
for i=1:length(FourTx)
    if FourTx(i) <=0 || FourTx(i) > 312 || FourTy(i) <=200 || FourTy(i) >512
    else
        FourTxx(t)=FourTx(i);
        FourTyy(t)=FourTy(i);
        t=t+1;
...
for i=1:length(SixTx)
    if SixTx(i) <=0 || SixTx(i) > 312 || SixTy(i) <=200 || SixTy(i) >512
    else
        SixTxx(t)=SixTx(i);
        SixTyy(t)=SixTy(i);
        t=t+1;
...
for i=1:length(EightTx)
    if EightTx(i) <=0 || EightTx(i) > 312 || EightTy(i) <=200 || EightTy(i) >512
    else
...
% Topon=Topo(201:512,1:312);
% Toponm=Topon;
% Toponmm=Topon;

% for z=1:100
    for i =1:length(AllTxx)
        x0 = [1; AllTxx(i); 3; AllTyy(i)-200; 3; 0; 0];
