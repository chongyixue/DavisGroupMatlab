function [eigeps2D, orbcha] = fese_Hirschfeld_tb_soc_pos(pixelnum, kz)
...
%         H1 = Hirschfeld_tb_hamiltonian_soc_pos_August2016(px, py, p1, p2, kz);
        H(:,:,m) = H1(:,:,1);
...
for k=0:floor(n/4)
    for i=1:20
        plot(pi+(kx(n/2+1+floor(n/4)+k)+ky(n/2+1-floor(n/4)+k)),...
            eigeps2D(n/2+1+floor(n/4)+k, n/2+1-floor(n/4)+k, i),...
            'Color',orbcha{n/2+1+floor(n/4)+k, n/2+1-floor(n/4)+k,i},...
...
line([0,3*pi],[0,0],'Color','k','LineStyle','-.','LineWidth',2)
xlim([0, 3*pi]);
ylim([-0.13, 0.13]);
...
for k=0:n/2
    for i=1:20
        plot(kxs(1+k),eigeps2D(1+k, n/2+1, i),'Color',...
            orbcha{1+k, n/2+1,i},'Marker','.','LineWidth',2, 'MarkerSize',12)
    end
end
ylim([-0.05, 0.05]);
...
for k=0:n/2
    for i=1:20
        plot(kys(1+k),eigeps2D(n/2+1, 1+k, i),'Color',...
            orbcha{n/2+1, 1+k,i},'Marker','.','LineWidth',2, 'MarkerSize',12)
    end
end
ylim([-0.05, 0.05]);
