function [y_new, p,gof, ci]=FeSe_lorentzian4(y,range,guess,low,upp)
% range is [left right]
...
for i=1:length(x)
    peak4(i)=gg(13)*(gg(14)/2)^2/((x(i)-gg(15))^2+(gg(14)/2)^2);
end
y_new = feval(p,x);
figure, plot(x,y,'.k',x,y_new,'-r',x,peak1,'-b', x,peak2,'-g', x,peak3,'-c', x,peak4,'-m','linewidth',2);
legend('data','fit','peak1', 'peak2', 'peak3', 'peak4');
% pause(0.5);
end