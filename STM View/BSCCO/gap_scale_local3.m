function local_scaled_spectra = gap_scale_local3(data,Cu_gap_map,Cu_index,Ox_index1,Ox_index2,Oy_index1,Oy_index2,avg_option)
...
if isempty(zero_ind1)        
        zero_ind1 = find(abs(x) == min(abs(x)),1);
end
sum_ind = 0;
...
for i=1:length(Cu_index)
    ...
    %if Cu_gap_map(Cu_index(i,1),Cu_index(i,2)) ~= 0
        ...
        gap_val = find_gap_vals(Cu_gap,x);
        %gap_val = find_gap_val(Cu_gap_map(Cu_index(i,1),Cu_index(i,2)),x);        
        gap_index = find(x == gap_val);
        sum_ind = sum_ind + abs(zero_ind1 - gap_index);
        %sum_ind = sum_ind + gap_index;
        Cu_gap_avg = Cu_gap_avg + Cu_gap;        
    end        
end
% number of intervals to use between zero index point and 1
Cu_gap_avg = Cu_gap_avg/Cu_good_count;
n = round(sum_ind/Cu_good_count);
n = n*10;
...
%e_space = abs(gap_min-x(zero_ind1))/n;
...
x_rescale = e1:(sgn*e_space):e2; x_rescale = x_rescale/abs(Cu_gap_avg)/1000;
...
% in new scale, determine how many point lie above and below the zero_ind
% point.  This will be used to center the spectra on a common point
len_abv_zero = length(zero_ind:length(x_rescale)) -1;
len_bel_zero = length(1:zero_ind);
l_x_rescale = length(x_rescale);
...
for i = 1:length(Cu_index)  
    ...
    if gap_val ~= 0        
        count = count + 1;
        %e_space = abs(gap_val-x(zero_ind1))/n;
...
%         xi = e1:(sgn*e_space):e2;
        ...
        y_Ox1 = interp1(x/(-sgn*gap_val),Y_Ox1,x_rescale*1000);
        y_Ox2 = interp1(x/(-sgn*gap_val),Y_Ox2,x_rescale*1000);
        y_Oy1 = interp1(x/(-sgn*gap_val),Y_Oy1,x_rescale*1000);
        y_Oy2 = interp1(x/(-sgn*gap_val),Y_Oy2,x_rescale*1000);      
        %  figure;
         % plot(x_rescale*1000,y_Ox1,'rx'); hold on; plot(x/-gap_val,Y_Ox1,'bx');
        %align the middle of local_scaled_spectra in the z-direciton with 
        %the middle of the newly interpolated spectra        
        pt1 = zero_ind - round(len_bel_zero/(l_x_rescale)*length(y_Ox1))+1;
        pt2 = zero_ind + round(len_abv_zero/(l_x_rescale)*length(y_Ox1));
        %pt1 = zero_ind - round(len_bel_zero/(l_x_rescale))*length(y_Ox1)+1
        %pt2 = zero_ind + round(len_abv_zero/(l_x_rescale))*length(y_Ox1)
        st_pt = min(pt1,pt2); end_pt = max(pt1,pt2);
