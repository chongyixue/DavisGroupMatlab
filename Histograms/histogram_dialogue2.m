...
revert_but = uicontrol(fh,'Style','pushbutton',...
                          ...
                          'units','centimeter',...
                          'Position',[9 1 2.5 1],...
                          'Callback',{@revert_Callback,c_min_revert,c_max_revert,layer});
...
bar(x,n./sum(n),1);
xlim([c_min_revert(layer) c_max_revert(layer)]);
%xlim([min(c_min_revert) max(c_max_revert)]);
          set(get(hist_axis,'XLabel'),'String','Value');
                  set(get(hist_axis,'YLabel'),'String','% Occurrence');
            
%%%%%%%%%%%%%%%%%%%%%%%%%Callback Functions%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function revert_Callback(hObject,evendata,c_min,c_max,layer)
        set(hist_axis,'XLim',[c_min(layer) c_max(layer)]);
        set(mini_edit,'String',num2str(c_min(layer)));
        set(maxi_edit,'String',num2str(c_max(layer)));
        set(axis_handle,'CLim',[c_min(layer) c_max(layer)]);
    end
