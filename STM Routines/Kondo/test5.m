...
% NOTE  that inead of 4V^2 only V^2 is used. So remember to divide fitted
% V value by 2 to get actual hybridization strength
kondo1 = (a^2*(k-b)^4 + e0 + l)/2 - (((a^2*(k-b)^4 + e0 - l)^2 + V^2)^0.5)/2;
kondo2 = (a^2*(k-b)^4 + e0 + l)/2 + (((a^2*(k-b)^4 + e0 - l)^2 + V^2)^0.5)/2;
...
s = optimset('MaxFunEvals',3000,'MaxIter',3000,'TolFun',1e-6);
[xx2,fval] = fminunc(gg,[40 0.5 -5 -3, 5],s);
% % n =2;
% l = -0.7414;
% V = 5;
% a= 40;
...
y1 = kondo_l2(k,xx2(1),xx2(2),xx2(3),xx2(4),xx2(5));
y2 = kondo_h2(k,xx2(1),xx2(2),xx2(3),xx2(4),xx2(5));
y3 = xx2(1)^2*(k-xx2(2)).^2 + xx2(3); % conduction band
...
%%
k=0.0:0.00001:0.3;
y1 = kondo_l2(k,xx2(1),xx2(2),xx2(3),xx2(4));
y2 = kondo_h2(k,xx2(1),xx2(2),xx2(3),xx2(4));
y3 = xx2(1)^2*k.^2 + xx2(2); % conduction band
...
b = 0.5;
e0 = -5;
yy1 = (a^2*(x-b).^n + e0 + l)/2 - (((a^2*(x-b).^n + e0 - l).^2 + V^2).^0.5)/2;
yy2 = (a^2*(x-b).^n + e0 + l)/2 + (((a^2*(x-b).^n + e0 - l).^2 + V^2).^0.5)/2;

figure; plot(x,yy1,'r'); hold on; plot(x,yy2,'b');