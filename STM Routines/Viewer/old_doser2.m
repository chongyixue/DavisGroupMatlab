...
fh=figure('Color',[1 1 1],'units','centimeter', ...
    'Position',[2,1.5,20,20]);

radius_edit = uicontrol(fh,'Style','Edit',...
                'String','10',...
                'units','centimeter',...
                'Position',[0.8 0.5 0.05 0.05]);
...
while count<2
    ...
    yi=yio/max(hs.disty)*hs.sy; % yi are in pixel
    xi=xio/max(hs.distx)*hs.sx;  
...
carp=get_carpet_from_cube(hs.cube,a,b,r);
[ssy,ssx]=size(carp);
kvec=linspace(0,sqrt(sum(a-b).^2),ssx);
