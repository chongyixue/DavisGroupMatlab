...
for i = 1:nr
    ...
    for j = 1:nc
        ...
        %y = G_data.ave; y = y';
        %y = squeeze(squeeze(G_data.map(113,13,:))); y = y';
        ...
        x0 = mean(x2(dy2 == min(dy2(1200:5000))));        
        gap_map1(i,j) = x0;
...
figure; pcolor(gap_map1); shading flat; colormap(Cmap.Defect1); axis equal; axis off;
%%
ft_gap = fourier_tr2d(gap_map1,'sine','','ft');
figure; pcolor(ft_gap); shading flat; colormap(Cmap.Defect1); axis equal; axis off;
%% find gap of current divided maps
% left gap edge
G_data = Gmod;
pt3 = 33; pt4 = 43;
...
for i = 1:nr
    ...
    for j = 1:nc
        y =  squeeze(squeeze(G_data.map(i,j,:))); y = y';   
        %y = G_data.ave; y = y';
        %y = squeeze(squeeze(G_data.map(16,120,:))); y = y';
        [p,S]= polyfit(x(pt3:pt4),y(pt3:pt4),4);        
        %y2 = polyval(p,x(pt1:pt2));
...
figure; pcolor(gap_map2); shading flat; colormap(Cmap.Defect1); axis equal; axis off;
%%
...
figure; pcolor(ft_gap_t); shading flat; colormap(Cmap.Defect1); axis off; axis equal;